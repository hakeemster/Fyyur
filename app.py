...
from models import app, db, moment, Show, Artist, Venue
...
def search_venues():
  ...
  for venue in venues:
      ...
      response['data'].append(single_ven)
...
def show_venue(venue_id):
  ...
  shows_query = db.session.query(Show, Artist, Venue).join(Artist).join(Venue).filter(Show.artist_id==Artist.id).filter(Show.venue_id==Venue.id).filter(Venue.id==venue_id).all()
  ...
  setattr(venue, 'genres', venue.genres.split(',')) # to convert the genre string back to an array
...
def create_venue_submission():
  ...
  try:
      ...
      seeking_talent = ''
      if (request.form.get('seeking_talent') == 'y'):  # My little archaic way to check for the checkbox value before assigning it to seek_venue
        seeking_talent = True
      else:
        seeking_talent = False
      seeking_description = request.form.get('seeking_description')
  
      venue = Venue(name=name, city=city, state=state, phone=phone, genres=genres, image_link=image_link, facebook_link=facebook_link, website_link=website_link, seeking_talent=seeking_talent, seeking_description=seeking_description)
...
def create_artist_submission():
  ...
  try:
      ...
      website_link = request.form.get('website_link')
      seeking_venue = ''
      if (request.form.get('seeking_venue') == 'y'):  # My little archaic way to check for the checkbox value before assigning it to seek_venue
        seeking_venue = True
      else:
        seeking_venue = False
      seeking_description = request.form.get('seeking_description')
...
def shows():
  ...
  shows = db.session.query(Venue.id, Venue.name, Artist.id, Artist.name, Artist.image_link, Show.start_time) \
      .filter(Artist.id == Show.artist_id, Venue.id == Show.venue_id).all()
  
  data = []
  for i in shows:
    # flash('Hello world' + str(i.artist_id) + str(i.venue_id) + str(i.start_time))
    # flash('Hello world' + str(i[0]) + " " + str(i[5]))

    temp = {}
    temp["venue_id"] = i[0]
    temp["venue_name"] = i[1]
    temp["artist_id"] = i[2]
    temp["artist_name"] = i[3]
    temp["artist_image_link"] = i[4]
    temp["start_time"] = i[5]
